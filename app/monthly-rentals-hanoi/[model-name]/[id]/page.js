...
const SimilarBikes = dynamic(
  () => import("../../../../components/SimilarBikes"),
  { ssr: false }
);
  ...
  return (
    <>
      <section className={styles.bikeDetail}>
        ...
        <h1>
          {bike.model} {bike.name} {bike.capacity}
          <span>cc</span>
        </h1>
        <div className={styles.deskLayout}>
          ...
          <div className={styles.details}>
            ...
            ></Image>
            <h2>
              Monthly rate - ₫{bike.monthPrice} <span>VND</span>
            </h2>
            <p className={styles.transmission}>Transmission: {bike.type}</p>
            <p>{bike.description}</p>
            <div className={styles.disclaimer}></div>
            <p>
              Monthly rentals have a maximum of 1000km/month and can not be
              taken outside of Hanoi city limits. This super cheap rate is
              provided to benefit expats and locals living and working in the
              city.
            </p>
            <p>
              {" "}
              All our rentals include helmets, a rack for your luggage, a phone
              holder for easy navigation, and secure rubber straps. Our garage
              is open 7 days a week and we are happy to maintain your bike
      ...
      <div className={styles.options}>
        ...
        <Link href={`/motorbike-rentals-vietnam/${bikeUrl}/${bike.id}`}>
          Get this {bike.model} for just ${bike.cityPrice}/day with weekly
        ...
        <Link href={`/motorbikes-for-rent-hanoi/${bikeUrl}/${bike.id}`}>
          We have daily rates from just ${bike.cityPrice}.!
        </Link>
        <Link href={`/motorbikes-for-rent-hanoi/${bikeUrl}/${bike.id}`}>
          <button className={styles.btn} style={{ marginTop: "1rem" }}>
            Check Daily Rates!
