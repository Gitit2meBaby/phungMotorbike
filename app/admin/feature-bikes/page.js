...
export default function FeatureBikesAdmin() {
    ...
    const uploadImages = async (files, bikeId, bikeModel, bikeName) => {
        ...
        const uploadPromises = files.flatMap(async (file, index) => {
            if (file && file.thumbImage && file.fullImage) {
                ...
                const folderPath = collectionType === 'featureBikes' ? 'featureBikes' : 'featureSaleBikes';
    ...
    const handleSubmitRental = async (e) => {
        ...
        if (!formData.model || !formData.name || !formData.capacity || !formData.cityPrice) {
    ...
    const handleSubmitSale = async (e) => {
        ...
        if (!formData.model || !formData.name || !formData.capacity || !formData.salePrice) {
            alert("Please fill in all required fields");
            return;
        }
        ...
        <>
            <section className={styles.admin}
                ...
                <div className={styles.btnWrapper}>
                    <button
                        className={collectionType === 'featureBikes' ? styles.btnActive : styles.btn}
                        onClick={() => handleCollectionChange('featureBikes')}
                    >
                        Featured Rentals
                    </button>
                    <button
                        className={collectionType === 'featureSaleBikes' ? styles.btnActive : styles.btn}
                ...
                {(formType === 'Add Bike' || formType === 'Edit Bike') && (
                    <form onSubmit={collectionType === 'featureBikes' ? handleSubmitRental : handleSubmitSale}>
