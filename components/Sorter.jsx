...
const Sorter = ({ onSort }) => {
    ...
    useEffect(() => {
        // Call onSort with the current sort method and direction
        const sortString = `${sortMethod}${sortMethod === 'price' ? (price === 'asc' ? '+' : '-') : (capacity === 'asc' ? '+' : '-')}`;
        onSort(sortString);
    }, [sortMethod, price, capacity, onSort]);
