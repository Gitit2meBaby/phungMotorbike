...
try {
  const serviceAccountRaw = await readFile(
    join(
      __dirname,
      "./phung-motorbike-firebase-adminsdk-cr6g0-3fd3f94690.json"
    ),
    "utf8"
  );
...
const processAndUploadBike = async (bikeData) => {
  ...
  if (typeof bikeData.Photo === "string") {
    ...
    console.warn(
      `Unexpected Photo value for bike ${bikeData.Name}: ${bikeData.Photo}`
    );
    imageIds = [];
  }
  ...
  for (let i = 0; i < imageIds.length; i++) {
    ...
    try {
      ...
      console.error(
        `Error processing image ${imageIds[i]} for bike ${bikeData.Name}:`,
        error
      );
    }
  }
...
const processAllBikes = async () => {
  ...
  let jsonData;
  try {
    const bikeDataRaw = await readFile(
      join(__dirname, "./spreadSheetData.json"),
      "utf8"
    );
